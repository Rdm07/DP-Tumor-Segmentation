...
def val_fn_epoch(classn = 1, val_fn = None, crit = None, val_loader = None):
    Pr = np.empty(shape = (500000, classn), dtype = np.float32)
    Or = np.empty(shape = (500000, classn), dtype = np.float32)
    Tr = np.empty(shape = (500000, classn), dtype = np.int32)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            ...
            if (epoch + 1) % args.check_after == 0:
                ...
                tra_val_loss.append([running_loss / N_tot, val_loss])
                ...
                if f1 > best_auc and epoch > 2:
                    ...
                    save_point = os.path.join(old_model_folder, 'checkpoint')
                    if not os.path.isdir(os.path.join(save_point)):
                        os.mkdir(save_point)

                    saved_model_fn = args.net_type + '_' + args.color + '_' + strftime('%m%d_%H%M')
                    torch.save(state, os.path.join(save_point, saved_model_fn + '_' + str(best_auc) + '_' + str(epoch) + '.t7'))
                    print('=======================================================================')
